...
class InstructorFinanceView(LoginRequiredMixin, View):
	...
	template_name = 'clinic/instructor_finance.html'
	...
	def post(self, request, instructor_id):
		...
		if form.is_valid():
			instructor_id = form.cleaned_data['instructor']
			instructor = Instructor.objects.get(id = instructor_id)
			return redirect('clinic:instructor_finance', instructor_id)

class SubmitPaymentsView(LoginRequiredMixin, View):
	 
	template_name = 'clinic/submit_payments.html'
...
class ConfirmInstructorPaymentView(LoginRequiredMixin, View):
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			return redirect('clinic:payments')
...
class InstructorsView(LoginRequiredMixin, View):
	...
	template_name = 'clinic/instructors.html'
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			return redirect('clinic:instructors')  # تغییر مسیر پس از ثبت موفق
		else:
...
class DeleteInstructorView(LoginRequiredMixin, View):
	...
	def get(self, request, instructor_id):
		instructor = Instructor.objects.get(id = instructor_id)
		instructor.delete()
		return redirect('clinic:instructors')
	
class CoursesView(LoginRequiredMixin, View):

	template_name = 'clinic/courses.html'
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			return redirect('clinic:courses')
